...
import fs from "fs";
import { CairoBytes31, hash } from "starknet";
dotenv.config({quiet:true});

async function main() {
    const list = JSON.parse(fs.readFileSync("./src/votersList.json").toString("ascii"));
    // each leaf is a string array : voterEmail
    const voters: Merkle.InputForMerkle[] = list.list.map((item:string[])=>{return new CairoBytes31(item[0]).toHexString()});
